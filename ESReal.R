...
dados <- read.csv2("C:\\Users\\wagne\\OneDrive\\Wagner - UFOP\\Nono Periodo\\Computa��o Evolucion�ria\\ATV2\\dadosESReal.txt")
...
desvioESReal1 <- sd(dados$FO[dados$Teste == "ESREAL1"])
desvioESReal2 <- sd(dados$FO[dados$Teste == "ESREAL2"])
# Média
media <- mean(dados$FO)
...
ESreal1 <- dados$FO[dados$Teste == "ESREAL1"]
ESreal2 <- dados$FO[dados$Teste == "ESREAL2"]

# Menor valor
minimoESReal1 <- min(dados$FO[dados$Teste == "ESREAL1"])
# Maior valor
maximoESReal1 <- max(dados$FO[dados$Teste == "ESREAL1"])

# Menor valor
minimoESReal2 <- min(dados$FO[dados$Teste == "ESREAL2"])
# Maior valor
maximoEsReal2 <- max(dados$FO[dados$Teste == "ESREAL2"])

 
# Maior valor
PiorGeradoESReal1 <- max(dados$FOPior[dados$Teste == "ESREAL1"])

 
# Maior valor
PiorGeradoESReal2 <- max(dados$FOPior[dados$Teste == "ESREAL2"])
...
t.test(ESreal1,ESreal2)
 
# Media1 < Media2
 
t.test(ESreal1, ESreal2, alternative = 'l')

mediaESReal1 <- mean(dados$FO[dados$Teste == "ESREAL1"])
mediaESREAL2 <- mean(dados$FO[dados$Teste == "ESREAL2"])

tempoESReal1 <- mean(dados$ms[dados$Teste == "ESREAL1"])
tempoESREAL2 <- mean(dados$ms[dados$Teste == "ESREAL2"])
 

# Media1 > Media2
 
t.test(ESreal1, ESreal2, alternative = 'g')
